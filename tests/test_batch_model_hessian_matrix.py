"""Test code for `batch_model_hessian_matrix()`."""
...
from src.hesse import batch_model_hessian_matrix
...
def test_batch_model_hessian_matrix_bilinear(bilinear: nn.Module, batch_size: int) -> None:
    ...
    with pytest.warns(UserWarning):
        hessian_matrix = batch_model_hessian_matrix(
            model=bilinear,
    ...
    expected_shape = torch.Size([batch_size]) + 2 * torch.Size([bilinear.B.weight.numel()])
...
def test_model_hessian_dict_double_bilinear(
    ...
    for batch in range(batch_size):
        hess_B1_B1 = hessian_matrix[batch, : (m * n), : (m * n)]
        hess_B1_B2 = hessian_matrix[batch, : (m * n), (m * n) :]
        hess_B2_B1 = hessian_matrix[batch, (m * n) :, : (m * n)]
        hess_B2_B2 = hessian_matrix[batch, (m * n) :, (m * n) :]
